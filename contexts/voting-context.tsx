...
export function VotingProvider({ children }: { children: React.ReactNode }) {
  ...
  // Hook de wagmi para obtener información de la wallet
  const { address, isConnected } = useAccount()

  // Actualizar la dirección de wallet cuando se conecte/desconecte
  useEffect(() => {
  ...
  const login = async (type: "voter" | "admin", credentials?: any): Promise<boolean> => {
    if (type === "admin") {
      ...
      // Para votantes, usar la dirección de wallet conectada si está disponible
      const walletAddress = address || "0x742d35Cc6634C0532925a3b8D4C0532925a3b8D4"
  ...
  const vote = async (candidateId: string, category: string): Promise<boolean> => {
    // Verificar que el usuario esté logueado, tenga tokens y wallet conectada
    if (!user || user.tokensRemaining <= 0 || user.votedCategories.includes(category) || !isConnected) {
      return false
    }
